...
| [`smoldata_mix_100m_2048.yaml`](configs/smoldata_mix_100m_2048.yaml) | 113.85M | 6.527M in a 200-step validation; up to 3.2752B | 4.7 GB measured | 5m12s cumulative training-step wall time / 200 steps on RTX 5090 | 5.911 validation-only | Public 50/30/20 FinePDFs-Edu, DCLM, FineWeb-Edu mixture; network required |
...
Measurements use JAX 0.10.2 and an RTX 5090; the offline smoke uses the development host CPU. The 200-step 114M validation was measured on 2026-07-31 with the shipped `2048 × batch 2 × accumulation 8` model geometry, BF16 compute, strict bin-packed segment resets, Muon, real streamed mixture data, and schema-3 checkpoints at steps 100 and 200. It retained the model/batch geometry while shortening the run and LR schedule, reducing the source-shuffle caps to 4,096 documents / 128 MiB, reducing the packed-row shuffle to 262,144 tokens, logging every 10 steps, checkpointing every 100 steps, and disabling scheduled eval/generation. The first compile-and-update step took 90.2 seconds, comprising 84.4 seconds of compile/device work and 5.8 seconds of data wait. Steps 20–200 had 1.101-second median wall time, 29,646 median valid tokens/s, 99.68% mean packing utilization, and a 4.700 GB process allocator peak. The final loss belongs to that deliberately compressed validation schedule, not the 100k-step recipe.

A separate one-step probe retained the shipped production buffers: the source shuffle reached 64,349 documents / 536.9 MB and the packed-row shuffle was configured for 4,096 rows / 8.39M tokens. It completed in 2m11s with 10,249,288 KiB (~9.77 GiB) maximum host RSS, no swap, and a 3.686 GB first-step device peak. These are measured fit/runtime checks on one machine, not guarantees. A completed 24 GB / RTX 4090 baseline and full 100k-step loss remain visibly pending rather than extrapolated.
