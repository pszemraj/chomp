...
| [`megalodon_100m_2048.yaml`](configs/pretrain/megalodon_100m_2048.yaml) | 113.85M | 3.2752B | 3.7 GB measured | Dry-run fit; 500-step validation predates the gated FFN | 5.187 eval at step 500 (pre-gate) | Public 50/30/20 FinePDFs-Edu, DCLM, FineWeb-Edu mixture; network required |
| [`megalodon_200m_2048.yaml`](configs/pretrain/megalodon_200m_2048.yaml) | 188.78M | 4.0940B | 4.9 GB measured | Dry-run fit | 10.847 sanity-only (pre-gate) | Same streamed mixture |
| [`megalodon_500m_2048.yaml`](configs/pretrain/megalodon_500m_2048.yaml) | 513.67M | 10.4806B | 10.7 GB measured | Dry-run fit | 10.845 sanity-only (pre-gate) | Same streamed mixture |
| [`megalodon_1b_2048.yaml`](configs/pretrain/megalodon_1b_2048.yaml) | 976.98M | 19.6512B | 20.3 GB measured | Dry-run fit | 10.808 sanity-only (pre-gate) | Same streamed mixture |
...
The Peak VRAM column was re-measured on 2026-08-05 after all four recipes moved to a gated FFN at param-matched width (see [Gated FFN and `ffn_hidden_dim`](docs/training.md#gated-ffn-and-ffn_hidden_dim)). These are exact-config `--dry-run` allocator peaks on the RTX 5090, taken under JAX's **default** allocator — no `XLA_PYTHON_CLIENT_PREALLOCATE=false`, no raised memory fraction — so each number is a fit against the default 25.25 GB pool rather than against the card. Every recipe fits that pool with room to spare; none requires an environment variable. Each dry run executed data loading, packing, the complete forward/backward graph, gradient checks, and the optimizer-state update. Peaks fell slightly versus the pre-gate measurements (100M 4.657 → 3.7 GB, 500M 11.1 → 10.7 GB, 1B 20.9 → 20.3 GB) because param-matching narrows `ffn_hidden_dim` by a third, shrinking the feed-forward intermediate.

Two caveats on the surrounding columns. The Final loss figures and the 500-step 100M validation described above were measured before the gated FFN and are retained as harness evidence, not as current-architecture losses. And a dry run performs no evaluation and emits no generation sample; periodic generation raises the peak of the following training step by a measured ~1.65 GB, which matters when a configuration sits near the pool ceiling — see [Device memory ceiling](docs/training.md#device-memory-ceiling). These measurements are evidence on one 32 GB machine, not guarantees. Sustained baselines, 24 GB/RTX 4090 fits for the larger templates, and full-schedule losses remain pending rather than being extrapolated.
...
`--dtype policy` writes a derived variant at megalodon-jax's bf16 policy dtypes — about half the size (755 MB → 379 MB for the 200M recipe) and inference-equivalent, since the forward pass casts those tensors to bf16 anyway. It is a genuinely mixed file: normalization, CEMA, and affine parameters stay fp32 because compute uses them at fp32. The fp32 export remains the canonical artifact. See [Export](docs/export.md) for the format, the manifest schema, when the variant is refused, and why there is no Hugging Face Transformers variant.
