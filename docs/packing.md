...
- Cycle size is `A*B` for training and `B` for evaluation (`A=1` there). Resume metadata records both effective lookaheads when evaluation data is enabled, because a configured change can be clamped away for training while still changing eval packing. That second lookahead follows the training knobs only under [`data.eval_packing: train`](config-reference.yaml); the default `onedoc` instrument pins its own, so nothing on this page reaches the eval rows (see [Evaluation](training.md#evaluation)).
...
A seeded, bounded shuffle between the packer and batch assembly decorrelates packed `[T]` windows. Its token/row sizing and effective-cycle formula are defined by [`data.window_shuffle_tokens` and `data.window_shuffle_max_rows`](config-reference.yaml). Resume remains exact because only upstream state at the window start and permutation progress are checkpointed, then the window is reconstructed and replayed deterministically. That compact state makes every row-reconstruction input resume-invariant while the shuffle is active; even warn mode refuses changes to token preparation, sequence/cycle geometry, packing mode or effective FFD knobs, and the window geometry or seed. Eval batches are never shuffled.
