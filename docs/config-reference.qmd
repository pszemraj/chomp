...
Always use `bfloat16` for compute and `float32` for parameters/accumulation.
:::
...
```bash
chomp-train --config configs/my_config.yaml \
...
|----------|-------|
| Required | No |
| Constraints | **Must differ from `eos_token_id`** |

::: {.callout-important}
## pad_token_id ≠ eos_token_id

Megalodon zero-masks pad embeddings and the training loop masks pad positions in loss.
If `pad_token_id == eos_token_id`, EOS tokens get zeroed and masked, breaking supervision.

If your tokenizer sets `pad_token_id == eos_token_id`, chomp fails validation after tokenizer
resolution. Choose a tokenizer with distinct pad/eos, or set
...
|----------|-------|
| Required | No |
| Constraints | Must be in `[0, vocab_size)`; **must differ from `pad_token_id`** |
...
#### `model.segment_masking` {#model.segment_masking}

```yaml
segment_masking: bool = false
```

Apply block-diagonal segment masking in attention for packed sequences.

| Property | Value |
|----------|-------|
| Required | No |
| Recommended | `false` (stream semantics) |

::: {.callout-note}
**Default: stream semantics.** The corpus is treated as a continuous token stream.
Segment IDs are still emitted for boundary loss masking.

Set to `true` for block-diagonal attention isolation across packed documents. Note
that Megalodon's ComplexEMA and TimestepNorm still leak across boundaries even with
segment masking enabled.
:::
...
Truncate documents to this many tokens before packing. `null` means no truncation.
...
Training loop configuration. Contains 12 fields.
...
Optimizer configuration for AdamW with linear warmup and cosine decay. Contains 5 fields.
...
Checkpoint directory. `null` uses `<run_dir>/checkpoints`.
...
| `model.pad_token_id != model.eos_token_id` | `model.pad_token_id must differ from model.eos_token_id` |
...
::: {.callout-important}
## pad_token_id Must Differ from eos_token_id

If your tokenizer uses the same ID for padding and EOS (common with GPT-2 style tokenizers),
chomp fails validation. Prefer a tokenizer with distinct pad/eos, or disable
`data.tokenizer.auto_set_special_tokens` and override `model.pad_token_id` explicitly:
...
For offline testing without network dependencies:

```yaml
# configs/debug_smoke.yaml — Minimal offline smoke test
model:
  backend: dummy
  vocab_size: 256
  d_model: 128
...
train:
  ...
  jit: true
  deterministic: true
  allow_cpu: true      # Allow CPU for CI
  log_every: 1
...
### Production Training (100M Megalodon)

For real pretraining on Zyda-2:

```yaml
# configs/zyda2_100m_2048.yaml — Production 100M Megalodon
model:
  backend: megalodon
  vocab_size: 32000              # Tokenizer vocab size (already 128-aligned)
  model_dim: 768
  num_layers: 12
  num_heads: 12
  z_dim: 384                     # model_dim // 2
  value_dim: 768                 # Same as model_dim
  ffn_hidden_dim: 3072           # 4 × model_dim
  cema_ndim: 16
  chunk_size: 512                # Must divide seq_len
  dropout: 0.0
  attention_dropout: 0.0
  hidden_dropout: 0.0
  pad_token_id: 3                # Distinct from eos_token_id
  bos_token_id: 1
  eos_token_id: 2
  init_mode: he
  use_checkpoint: true           # Gradient checkpointing
  # Precision policy (NEVER use fp16)
  param_dtype: float32
  compute_dtype: bfloat16
  accum_dtype: float32
  softmax_dtype: float32
...
data:
  ...
  text_key: text
  packing_mode: bin
  packing_buffer_docs: 256       # >= batch_size × grad_accum
  grain_prefetch: 2
  mask_boundary_loss: true
  train_on_eos: true
...
train:
  ...
  steps: 10000
  batch_size: 2
  seq_len: 2048                  # Must be divisible by chunk_size
  grad_accum: 8                  # Effective batch = 16
  jit: true
  deterministic: true
  allow_cpu: false               # Require GPU
  log_every: 25
  eval_every: 2500
...
optim:
  ...
  weight_decay: 0.01
  grad_clip_norm: 1.0
  warmup_steps: 100              # Must be < steps
  min_lr_ratio: 0.0
...
logging:
  ...
  wandb:
    ...
    project: chomp
    mode: online
    tags: ["zyda2", "100m", "seq2048", "chunk512"]
...
**train** (12 fields): [`seed`](#train.seed), [`steps`](#train.steps), [`batch_size`](#train.batch_size), [`seq_len`](#train.seq_len), [`grad_accum`](#train.grad_accum), [`jit`](#train.jit), [`deterministic`](#train.deterministic), [`allow_cpu`](#train.allow_cpu), [`log_every`](#train.log_every), [`eval_every`](#train.eval_every), [`profile`](#train.profile), [`profile_dir`](#train.profile_dir)

**optim** (5 fields): [`lr`](#optim.lr), [`weight_decay`](#optim.weight_decay), [`grad_clip_norm`](#optim.grad_clip_norm), [`warmup_steps`](#optim.warmup_steps), [`min_lr_ratio`](#optim.min_lr_ratio)
