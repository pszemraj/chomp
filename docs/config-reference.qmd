...
`--override optim.muon_consistent_rms=0.2`). Use quotes to force a string.
...
Optimizer configuration for AdamW or Muon with linear warmup and cosine decay. Contains 19 fields.
...
#### `optim.muon_lr_scale` {#optim.muon_lr_scale}
...
`optim.lr * optim.muon_lr_scale`. The default (`100.0`) reflects the best
1k-step Muon sweep result so far; see `docs/optimization.md` for details.
...
#### `optim.muon_weight_decay_mult` {#optim.muon_weight_decay_mult}
...
#### `optim.muon_momentum` {#optim.muon_momentum}
...
#### `optim.muon_ns_steps` {#optim.muon_ns_steps}
...
#### `optim.muon_nesterov` {#optim.muon_nesterov}
...
#### `optim.muon_consistent_rms` {#optim.muon_consistent_rms}

```yaml
muon_consistent_rms: float | null = null
```

Muon RMS scaling factor. Set to `0.2` to enable consistent RMS scaling (Optax
recommendation) or leave `null` to match the earlier Muon behavior.
...
#### `optim.muon_allow_all_2d` {#optim.muon_allow_all_2d}
...
#### `optim.muon_allow_tied_embed` {#optim.muon_allow_tied_embed}
...
#### `optim.adam_b1` {#optim.adam_b1}

```yaml
adam_b1: float = 0.9
```

AdamW beta1 coefficient. Applies to the full model for `optim.name=adamw` and
to the non-Muon parameter group for `optim.name=muon`.
...
#### `optim.adam_nesterov` {#optim.adam_nesterov}
...
`optim.muon_nesterov` to avoid accidentally running NadamW on non-Muon params.
...
**optim** (19 fields): [`name`](#optim.name), [`lr`](#optim.lr), [`muon_lr_scale`](#optim.muon_lr_scale), [`weight_decay`](#optim.weight_decay), [`muon_weight_decay_mult`](#optim.muon_weight_decay_mult), [`grad_clip_norm`](#optim.grad_clip_norm), [`warmup_steps`](#optim.warmup_steps), [`decay_steps`](#optim.decay_steps), [`min_lr_ratio`](#optim.min_lr_ratio), [`muon_momentum`](#optim.muon_momentum), [`muon_ns_steps`](#optim.muon_ns_steps), [`muon_nesterov`](#optim.muon_nesterov), [`muon_consistent_rms`](#optim.muon_consistent_rms), [`muon_allow_all_2d`](#optim.muon_allow_all_2d), [`muon_allow_tied_embed`](#optim.muon_allow_tied_embed), [`adam_b1`](#optim.adam_b1), [`adam_b2`](#optim.adam_b2), [`adam_eps`](#optim.adam_eps), [`adam_nesterov`](#optim.adam_nesterov)
