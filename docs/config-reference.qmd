...
Optimizer configuration for AdamW with linear warmup and cosine decay. Contains 6 fields.
...
|----------|-------|
| Required | No |
| Constraints | Must be ≥ 0; **must be < `optim.decay_steps` (effective)** |

::: {.callout-warning}
If `warmup_steps >= optim.decay_steps (effective)`:
`optim.warmup_steps (X) must be < optim.decay_steps (Y)`
:::
...
Cosine decay horizon (in steps). `null` uses `train.steps`. Set explicitly if you
plan to resume with a different `train.steps` but keep the same LR schedule.

| Property | Value |
|----------|-------|
| Required | No |
| Constraints | Must be positive when set; must be > `optim.warmup_steps` |
...
| `optim.warmup_steps < optim.decay_steps` (effective) | `optim.warmup_steps (X) must be < optim.decay_steps (Y)` |
...
**optim** (6 fields): [`lr`](#optim.lr), [`weight_decay`](#optim.weight_decay), [`grad_clip_norm`](#optim.grad_clip_norm), [`warmup_steps`](#optim.warmup_steps), [`decay_steps`](#optim.decay_steps), [`min_lr_ratio`](#optim.min_lr_ratio)
