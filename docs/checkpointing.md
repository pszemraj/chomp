...
- **A selected checkpoint owns its recorded semantics.** Its embedded config and source identity are the prior-state authority for resume compatibility and the default for generation. `config_resolved.json` is the immutable run-start record only: it locates a run directory and preserves provenance, and is never substituted for a checkpoint's own config. A step directory carrying no config metadata is damaged, and generation or export from it requires an explicit `--config`. The requested resume config still flows through `warn` or `strict`, and an explicit generation config remains the highest-priority user choice.
...
Warn mode can change packing semantics only when packed-window shuffling is disabled. With shuffling enabled, Grain checkpoints the producer at the current window's start plus a packed-row cursor, so resume requires the recipe that reconstructs those rows to stay fixed: sequence length, packing-cycle size, tokenizer execution and document preparation, packing mode and its effective FFD knobs, and packed-window geometry and seed. Applying the saved cursor to rows rebuilt by a different recipe would silently replay or skip source documents, so these mismatches are hard errors in both compatibility modes. Use `--init-from` when a new packing phase should start from the old parameters under the maintained shuffled pipeline.

The owned Hugging Face document shuffle has the same compact checkpoint shape one layer earlier: unshuffled source state at the current document window's start plus a document cursor. While that shuffle is active, source selection and partitioning plus shuffle enablement, buffer geometry, and seed must match in both compatibility modes. `data.repeat` remains under ordinary warn/strict policy because it changes future exhaustion behavior rather than reconstruction of the saved document window. An active packed-row shuffle independently binds the source recipe even when HF document shuffling is disabled.
...
Because no data or optimizer state is restored, warm start deliberately imposes **no** config compatibility check: packing mode, corpus, batch shape, optimizer, and schedule may all differ. Two things are still enforced. Every parameter the new config needs must be present in the checkpoint at the same shape and dtype; Orbax's restore is permissive here by design — that permissiveness is what lets warm start ignore the saved optimizer state — so chomp checks the restored tree itself and names the offending parameters when it refuses. Parameters present in the checkpoint but unused by the new config are dropped silently, so a narrower architecture warm starts from the subset it shares. The source checkpoint's tokenizer identity must equal the new run's, which is checked explicitly because it is the one mismatch no structural check can see — a different tokenizer of the same vocabulary size restores cleanly and then trains on embedding rows that mean something else. Checkpoints with no recorded tokenizer identity are refused rather than assumed compatible, before the new run directory is created.

A tokenizer refusal names the manifest fields that differ — `packages` for a rebuilt environment, `files` for a genuinely different vocabulary — whenever both runs' `tokenizer/identity.json` snapshots are readable and the source's still hashes to what its checkpoint recorded; otherwise it falls back to printing the two identity hashes. A warm start that is refused after its run directory was created removes that directory again, so the retry fails on the real problem rather than on "run dir already exists".
