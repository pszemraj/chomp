...
The manager and data iterator close on every exit path. Orbax waits for asynchronous writes and releases its checkpointer, metadata stores, and deleter; Grain stops prefetch workers and closes the underlying Hugging Face stream. Datasets 5.0.0 is pinned because it provides the remote-Parquet thread-shutdown workaround for successful processes that stop mid-shard. For a single-source Parquet stream, Chomp closes the generator, applies Datasets' bounded shutdown grace when the builder requests it, then releases its Arrow-backed dataset and iterator references while CPython is still live. Local and non-Parquet streams do not wait. Orbax enforces `checkpoint.max_to_keep` for retained checkpoints.
