...
`chomp generate` accepts a run directory or checkpoint step directory, restores the stored parameters and resolved config, and uses the run-pinned tokenizer described in [Data Pipeline: tokenization](data_pipeline.md#tokenization) when available. Generation recomputes that tokenizer's effective manifest identity and requires it to match the selected checkpoint before encoding the prompt; a checkpoint whose metadata records no identity is refused rather than encoded under an unproven vocabulary. Set `--temperature 0` for greedy decoding; seeded sampling is the default. Run `chomp generate --help` for the option list.
