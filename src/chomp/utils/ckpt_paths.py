...
from chomp.config import Config, build_config, load_config
...
def _read_meta_config(step_dir: Path) -> dict[str, Any] | None:
    ...
    """
    if not (step_dir / "meta" / "metadata").exists():
        return None
...
def read_checkpoint_meta(step_dir: Path) -> dict[str, Any]:
    ...
    """
    meta_path = step_dir / "meta" / "metadata"
...
def load_config_for_checkpoint(*, step_dir: Path, config_override: str | None) -> Config:
    """Load config from an explicit override or the checkpoint's own metadata.

    ``config_resolved.json`` is deliberately not a fallback. It records the
    configuration a run *started* with, which is not necessarily the one the
    selected checkpoint was written under; using it would hand a model whose
    static semantics silently disagree with the arrays being restored. Every
    checkpoint chomp writes carries its own config, so a step directory without
    one is damaged, and asking for ``--config`` beats guessing.

    :param Path step_dir: Checkpoint step directory.
    :param str | None config_override: Optional path to override config file.
    :return Config: Loaded configuration.
    :raises FileNotFoundError: If the checkpoint carries no config metadata.
    """
    if config_override:
    ...
    if data is None:
        raise FileNotFoundError(
            f"Checkpoint {step_dir} carries no config metadata. Pass --config with the "
            "config these weights were trained under."
        )
